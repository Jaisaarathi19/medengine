...
const roles = [
  {
    ...
    description: 'Register as a new patient and start your health journey with our AI-powered care platform.',
    href: '/create-patient',
    color: 'gradient',
    isSpecial: true,
  },
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            {roles.map((role, index) => (
              ...
              >
                <Link href={role.href}>
                  <motion.div
                    className={`group h-full p-8 rounded-2xl backdrop-blur-sm border-2 shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer relative ${
                      role.isSpecial 
                        ? 'bg-gradient-to-br from-blue-500/10 to-purple-600/10 border-blue-300/50 hover:border-blue-400'
                        : 'bg-white/80 border-gray-200/50'
                    }`}
                    whileHover={{ 
                      scale: 1.03, 
                      y: -8,
                      borderColor: role.isSpecial ? 'rgb(59 130 246)' :
                                   role.color === 'blue' ? 'rgb(59 130 246)' :
                                   role.color === 'green' ? 'rgb(34 197 94)' :
                                   role.color === 'purple' ? 'rgb(147 51 234)' :
                                   'rgb(239 68 68)',
                      boxShadow: role.isSpecial ? "0 25px 50px -12px rgba(59, 130, 246, 0.25)" : "0 25px 50px -12px rgba(0, 0, 0, 0.2)"
                    }}
                    whileTap={{ scale: 0.97 }}
                  >
                    {role.isSpecial && (
                      <motion.div
                        className="absolute -top-3 -right-3 bg-gradient-to-r from-yellow-400 to-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg"
                    ...
                    <div className="flex flex-col h-full">
                      ...
                      <div className="flex-grow">
                        <h3 className={`text-2xl font-bold mb-2 transition-colors ${
                          role.isSpecial ? 'text-blue-900 group-hover:text-blue-700' : 'text-gray-900 group-hover:text-blue-600'
                        }`}>
                          {role.title}
                        </h3>
                        <p className={`text-sm font-semibold mb-4 ${
                          role.isSpecial ? 'text-blue-600' :
                      ...
                      {/* Animated Button */}
                      <motion.div
                        className={`inline-flex items-center justify-center px-6 py-3 font-semibold rounded-xl shadow-lg group-hover:shadow-xl transition-all duration-300 ${
                          role.isSpecial 
                        ...
                        whileHover={{ 
                          scale: 1.05,
                          backgroundImage: role.isSpecial 
                            ? 'linear-gradient(to right, rgb(37 99 235), rgb(126 34 206))'
                            : role.color === 'blue' ? 'linear-gradient(to right, rgb(59 130 246), rgb(37 99 235))' :
                        ...
                        whileTap={{ scale: 0.95 }}
                      >
                        {role.isSpecial ? 'Register Now' : 'Access Portal'}
                        <motion.svg
